...
{
	[LkRecord]
	[TableView("GROUP_APPMENU", true)]
	[LkDataBase("BASIC")]
	[Serializable]
	public class GroupAppmenu_Record : RecordBase{
		public GroupAppmenu_Record(){}
		/*欄位資訊 Start*/
		string _UUID=null;
		...
		public GroupAppmenu_Record Clone(){
			try{
				return this.Clone<GroupAppmenu_Record>(this);
			}
			catch (Exception ex){
		...
		public GroupAppmenu gotoTable(){
			try{
				var dbc = LK.Config.DataBase.Factory.getInfo();
				GroupAppmenu ret = new GroupAppmenu(dbc,this);
				return ret;
			}
			catch (Exception ex){
				log.Error(ex);LK.MyException.MyException.Error(this, ex);
