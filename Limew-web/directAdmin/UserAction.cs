...
public class UserAction : BaseAction
{
    ...
    public JObject ValidateCode(string code, Request request)
    {
        ...
        try
        {
            ...
            */
            if (Limew.Parameter.Config.ParemterConfigs.GetConfig().GraphicsCertification == false)
            {
    ...
    public JObject logon(string company, string account, string password, HttpRequest request)
    {
        ...
        { /*權限檢查*/
            ...
            var drMenu = mBasic.getAuthorityMenuVByAttendantUuid(drAttendantV.First().UUID, Limew.Parameter.Config.ParemterConfigs.GetConfig().InitAppUuid);
            ...
            {
                /*本身是身份認證中心*/
                Limew.Controller.Model.Cloud.CloudModel cMod = new Limew.Controller.Model.Cloud.CloudModel();
                //cMod.getActiveConnection_By_Uuid
                Limew.Model.Basic.Table.Record.ActiveConnection_Record newAc = new Limew.Model.Basic.Table.Record.ActiveConnection_Record();
